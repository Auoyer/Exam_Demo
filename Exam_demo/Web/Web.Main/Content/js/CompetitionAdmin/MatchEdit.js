            ...
            if (data.IsSuccess) {
                ...
                if (type == 1) {
                    ...
                    if (type == 2) {
                        ...
                        $('#txtPFStartTime').attr('onfocus', 'WdatePicker({ minDate: \'#F{$dp.$D(\\\'txtFSEndTime\\\')}\' , dateFmt: "yyyy-MM-dd HH:mm"})');            // 评分开始时间小于复赛结束时间
                        // 初赛成绩发布开始时间大于初赛时间，小于复赛开始时间
                        $('#txtCSCJDSFB').attr('onfocus', 'WdatePicker({ minDate: \'#F{$dp.$D(\\\'txtBSEndTime\\\')}\', dateFmt: "yyyy-MM-dd HH:mm",maxDate: \'#F{$dp.$D(\\\'txtFSStartTime\\\')}\'})');
                        $('#txtFSCJDSFB').attr('onfocus', 'WdatePicker({ minDate: \'#F{$dp.$D(\\\'txtPFEndTime\\\')}\', dateFmt: "yyyy-MM-dd HH:mm" })');            // 复赛成绩发布晚于复赛结束时间
                        // 设置为必填
                        $('#txtFSStartTime').addClass('IsRequired');
                        $('#txtPFStartTime').addClass('IsRequired');
                        $('#txtFSEndTime').addClass('IsRequired');
                        $('#txtPFEndTime').addClass('IsRequired');
