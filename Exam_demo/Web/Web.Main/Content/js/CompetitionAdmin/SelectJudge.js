    ...
    $('#btnSelectJudge').click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#popDisplayJudeg').find('input[type=text]').keyup(function () {
    ...
    pageHelper.Init({
        ...
        bind: function (data) {
            ...
            $(data.Data).each(function (index, dom) {
                ...
                var trHtml = "";
                trHtml += "<tr>";
                if (judgeIdArr.length > 0) {
                    var flag = false;
                    for (var i = 0; i < judgeIdArr.length; i++) {
                        if (judgeIdArr[i] == dom.Id) {
                            trHtml += "<td><input class=\"icheck\" type=\"checkbox\" id=\"chkUser_{5}\" val=\"{5}\" name=\"chbSelectJudge\" disabled=\"disabled\" checked='checked'></td>";
                            flag = true;
                            break;
                        }
                    }
...
function SelectJudgeCallBack() {
    ...
    if (index != 0) {
        ...
        for (var i = 0; i < model.length; i++) {
            ...
            html += "<td title='" + model[i].AccountNo + "'>" + model[i].AccountNo.ToLeft(10) + "</td>";
            html += "<td title='" + model[i].Name + "'>" + model[i].Name.ToLeft(10) + "</td>";
            html += "<td title='" + model[i].Email + "'>" + model[i].Email.ToLeft(12) + "</td>";
            html += "<td title='" + model[i].CollegeName + "'>" + model[i].CollegeName.ToLeft(8) + "</td>";
