$(document).ready(function () {
    ...
    $("#btnUpLoad").click(function () {
        ...
        $.ajaxFileUpload({
            url: "/CompetitionAdmin/User/Import?random=" + Math.random(),
            secureuri: false,
    ...
    pageHelper.Init({
        ...
        bind: function (data) {
            ...
            $(data.Data).each(function (index, dom) {
                ...
                var trHtml = "";
                trHtml += "<tr>";
                trHtml += "<td><input class=\"icheck\" type=\"checkbox\" id=\"chkUser_{6}\" name='test'></td>";
                trHtml += "<td><div title=\"{1}\" class=\"ellipsis\">{1}</div></td>";
                trHtml += "<td><div title=\"{2}\" class=\"ellipsis\">{2}</div></td>";
                trHtml += "<td>{3}</td>";
                trHtml += "<td><div title=\"{4}\" class=\"ellipsis\">{4}</div></td>";
...
function Add() {
    ...
    $('#btnCancel').hide();
    $('#btnQX').show();
    dialogHelper.Show("popAddInfo", 650);
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.IsSuccess) {
                ...
                $("#lblCode").html(data.Data.AccountNo);
...
function GetRegList() {
    if ($('#selAudit').val() == 0)
        $('#divAuditDisplay').show();
    else
        $('#divAuditDisplay').hide();
