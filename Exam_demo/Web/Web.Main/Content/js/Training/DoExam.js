...
$(function () {
    ...
    GetPaperDetailEnter(Id, LibraryId)
    ...
    $(".as2").unbind("click").click(function () {
        ...
        if (prevId == 0) {
            var question = $("#question_list_content .cur-answer").attr("tag");
            var type = $("#question_list_content .cur-answer").attr("ty");
            if (Type != 1) {
            ...
            var question = $("#question_list_content .cur-answer").attr("tag");// $("#hdQuestionId").val();
            var type = $("#question_list_content .cur-answer").attr("ty");
            if (Type != 1) {
                UpdateExamAnswer(Id, question, type);
            }
...
function GetPaperDetailList(Id, LibraryId, questionType, pageIndex, questionTypeId, optAsync) {
    ...
    //点击题型按钮时保存题目
    var question = $("#question_list_content .cur-answer").attr("tag");
    var type = $("#question_list_content .cur-answer").attr("tyid");
    var type3 = $("#question_list_content .cur-answer").attr("ty");
        ...
        success: function (data) {
            ...
            if (data != null && data != "") {
                ...
                if (data.Flag) {
                    ...
                    if (type != 1) {
                        //当前题目颜色
                        $("#question_list_content > a").removeClass("cur-answer");
                        $("#li_question_" + data.Current.Value).addClass("cur-answer");
                    }
                    else {
                        //当前题目颜色
                        $("#question_list_content > a").removeClass("cur-answer");
                ...
                var type = $.getUrlParam("type");
                if (type != 1) {
                    $(".li_question_" + questionId).addClass("cur-answer");
                    var ii = $("#question_list_content .cur-answer").html();
                    $("#itm font").html("第" + ii + "题");
                }
                else {
                    $(".li_question_" + questionId).addClass("cur-answer");
                    var ii = $("#question_list_content .cur-answer").html();
...
function UpdateExamAnswer(Id, question, nextType, bo) {
    ...
    if (Type != 1) {
        ...
        question = $("#question_list_content .cur-answer").attr("tag");
        //更新答案
        $.ajax({
...
function daojishi(Id) {
    ...
    window.setInterval(function () {
        if (time >= 0) {
            var hh = parseInt(time / 60 / 60, 10); //计算剩余的小时数  
    ...
    //每五分钟访问一次后台，校准时间
    //window.setInterval("GetTimes(true," + Id + ")", 10 * 60 * 1000);
}

//倒计时选相关方法2
function daojishi2(time) {
...
//加载左侧列表
function GetPaperDetailEnter(Id, LibraryId) {
    var userId = $("#hdUserId").val();
    $.ajax({
        url: "/CompetitionUser/EheoryExamine/GetPaperEnter",
        async: false,
        type: "POST",
        data: {
            Id: Id,
        ...
        success: function (data) {
            ...
            if (data != null && data != "") {
                ...
                if (type != 1) {
                    //当前题目颜色
                    $("#question_list_content > a").removeClass("cur-answer");
                    $("#li_question_" + firstShow.Current.Value).addClass("cur-answer");
                }
                else {
                    //当前题目颜色
                    $("#question_list_content > a").removeClass("cur-answer");
                ...
                //答案
                answer_cache = new Array();
                if (firstShow.StructType == 1 || firstShow.StructType == 2 || firstShow.StructType == 3) {
