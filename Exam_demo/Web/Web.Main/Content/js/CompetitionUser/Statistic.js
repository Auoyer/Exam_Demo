...
function GetModel() {
    // 加载编辑页面
    $.ajax({
        url: '/CompetitionUser/StatisticAlanalysis/GetModel',
        data: {
            id: $('#hideMatchId').val()
        },
        async: false,
...
//function GetDetail() {
//    pageHelper.Init({
//        url: '/CompetitionJudges/StatisticAlanalysis/GetDetail',
//        type: "POST",
...
//                    trHtml += "<td>{1}<font class=\"d_zhu\">注</font></td>";
//                }
//                else {
//                    trHtml += "<td>{1}</td>";
//                }
...
function GetScore() {
    pageHelper.Init({
        url: '/CompetitionUser/StatisticAlanalysis/GetDetail',
        type: "POST",
        pageDiv: "#ScorePages",
        async: false,
        data: {
            ...
            if (data.Data == "" || data.Data == null) {
                ...
                if (data.Data[0].Type == 1) {
                    ...
                    $(data.Data).each(function (index, dom) {
                        ...
                        trHtml += "</tr>";
                        trHtml += "</tbody>";
                        var pcode = dom.ProvinceCode;
                        var ccode = dom.CityCode;
                        var pc = getCodeName(pcode, ccode);
                        var p, c;
                        if (pcode == "" || ccode == 0) {
                            p = "";
                        }
                        ...
                        if (pcode != "" && pcode != 0 && ccode != "" && ccode != 0) {
                            p = $.trim(pc.substring(0, pc.indexOf("-")));//取省
                            c = $.trim(pc.substring(pc.indexOf("-") + 1));//取市
                        }
...
function GetStatistic() {
    ...
    var series = [];
    $.ajax({
        url: "/CompetitionUser/StatisticAlanalysis/GetUserMatchScoreList",
