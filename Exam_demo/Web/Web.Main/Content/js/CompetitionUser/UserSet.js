        ...
        bind: function (data) {
            ...
            $("#userList .userNames").each(function (index, item) {
                ...
                for (var i = 0; i < arrName.length; i++) {
                    str += '<a onclick="OpenInfo(' + arrIds[i] + ')">' + arrName[i] + '</a>';
                }
                $(this).html(str);
            })
        }
        ...
        success: function (data) {
            ...
            $(data.Data).each(function (index, dom) {
                ...
                $(dom.List).each(function (index1, dom1) {
                    html += "<dd rel='" + dom1.UserId + "'><span>" + dom1.UserNames + "</span><a onclick='DeleteSelectUser(this)'></a></dd>";
                })
                html += "</dl>";
            });
...
function AddUser(userId, obj) {
    ...
    $('#alreadyGroup dt.select').parent().append('<dd rel="' + userId + '"><span>' + userName + '</span><a onclick="DeleteSelectUser(this)"></a></dd>');
...
function IsSelectedLoadBackColor() {
    ...
    // 先清空全部背景色
    $('#userList tr td').css('background-color', '#fefefe');
    ...
    $('#alreadyGroup dd').each(function (index, dom) {
        ...
        var rightUserId = $(this).attr('rel');
        // 循环左侧
        $('#userList tr').each(function (index1, dom1) {
...
function btnUpLoadClick() {
    ...
    if (filePath.length > 0) {
        ...
        if (fileType == ".xls" || fileType == ".xlsx") {
            ...
            dialogHelper.Error({ content: "只能选择xls文件!" });
            return;
        }
    }
