...
$(function () {
    ...
    $(".as1").unbind("click").click(function () {
        ...
        //$(".as2").attr('disabled', true);
        //$("#btnPrev").attr('disabled', true); //TextContent2
        //$("#btnPrev").addClass('btndisable');//00a2e2
    ...
    var matchstate = $.getUrlParam("MatchState");
...
function GetPaperDetailList(Id, LibraryId, questionType, pageIndex, questionTypeId, optAsync) {
    ...
    $.ajax({
        url: "/Admin/Paper/GetPaperDetailList",
        async: false,
        type: "POST",
        data: {
            ...
            if (data != null && data != "") {
                ...
                if (data.QuestionTypes != null && data.QuestionTypes.length > 0) {
                    ...
                    $(data.QuestionTypes).each(function (index, dom) {
                        ...
                        htmls += "<a data-value=\"" + dom.strIdList + "\" name=\"type_" + dom.strIdList + "\"" + (index == 0 ? "class=\"on\"" : "") + " onclick=\"javascript:UpdateExamAnswer2(" + Id + "," + GetTypeId(dom.TypeName) + ");GetPaperDetailList(" + Id + "," + LibraryId + "," + GetTypeId(dom.TypeName) + ",1,'" + dom.strIdList + "',true);GetQuestion(" + Id + "," + LibraryId + "," + GetTypeId(dom.TypeName) + ",-1)\" >" + dom.TypeName + "</a>";
                    });
...
function UpdateExamAnswer(Id, question, nextType, bo) {
    ...
    if (Type != 1) {
        ...
        if (check_answer_flag) {
            ...
            //更新答案
            $.ajax({
                url: "/Admin/Paper/UpdateExamAnswer",
                type: "POST",
                async: false,
                dataType: "json",
                crossDomain: false,
...
//更新答案
function UpdateExamAnswer2(Id, nextType) {
    var Type = $.getUrlParam("type");
        ...
        success: function (data) {
            if (data.Flag) {
                dialogHelper.Success({
                    content: "试卷提交成功！",
                    success: function () {
                        location.href = "Admin/MatchStart/Detail/" + MatchId;
                    },
                    cancle: function () {
                        location.href = "Admin/MatchStart/Detail/" + MatchId;
                    }
...
function DownLoad(path, name, Id) {
    ...
    location.href = "/Admin/Paper/DownloadFile?path=" + encodeURIComponent(path) + "&name=" + encodeURIComponent(name) + "&randomId=" + Math.random();
...
function LookOver(Id) {
    var UserId = $.getUrlParam("UserId");
    $.ajax({
        url: "/Admin/Paper/GetPaperDetail",
        async: false,
        type: "POST",
        data: {
            Id: Id,
